#!/bin/sh
# Genba Memory — sets the passwords of the database roles created by db/schema.sql from
# secret files (OPS-15 §4.2). The passwords are read from the same files that
# database_url / worker_database_url embed; nothing is echoed.
set -eu
